import { createEmptyGoalContributions, createGoalPeriod, normalizeGoalContributions } from '../lib/goals';
import type { AppState } from '../types/domain';

const now = new Date();

const isoAt = (dayOffset: number, hour: number, minute: number) => {
  const value = new Date(now);
  value.setDate(value.getDate() + dayOffset);
  value.setHours(hour, minute, 0, 0);
  return value.toISOString();
};

const localDateKeyAt = (dayOffset: number) => {
  const value = new Date(now);
  value.setDate(value.getDate() + dayOffset);

  return `${value.getFullYear()}-${String(value.getMonth() + 1).padStart(2, '0')}-${String(
    value.getDate(),
  ).padStart(2, '0')}`;
};

export const mockState: AppState = {
  telegramName: 'Гость смены',
  shift: {
    id: 'shift-today',
    startedAt: now.toISOString(),
    dayLabel: 'Вечерняя смена',
    leftoversChecked: false,
    closingPhotosChecked: false,
  },
  tasks: [
    {
      id: 'task-1',
      title: 'Обновить витрину десертов',
      assignee: 'Маша',
      assigneeId: 'emp-masha',
      points: 20,
      rewardAmount: 400,
      status: 'assigned',
      dueLabel: 'до 20:00',
    },
    {
      id: 'task-2',
      title: 'Промаркировать заготовки',
      assignee: 'Антон',
      assigneeId: 'emp-anton',
      points: 15,
      rewardAmount: 350,
      status: 'done',
      dueLabel: 'до конца смены',
      completedAt: now.toISOString(),
    },
    {
      id: 'task-3',
      title: 'Проверить кофе-зону',
      assignee: 'Илья',
      assigneeId: 'emp-ilya',
      points: 10,
      rewardAmount: 250,
      status: 'accepted',
      dueLabel: 'сделано',
      completedAt: now.toISOString(),
      acceptedAt: now.toISOString(),
    },
  ],
  losses: {
    spoilage: 1200,
    staffMeal: 450,
    rd: 300,
    updatedAt: now.toISOString(),
  },
  handoffItems: [
    {
      id: 'handoff-1',
      area: 'kitchen',
      title: 'Соус том-ям на утро',
      criticality: 'high',
      checked: true,
      reason: 'Готово и промаркировано',
    },
    {
      id: 'handoff-2',
      area: 'kitchen',
      title: 'База для рамэна',
      criticality: 'medium',
      checked: false,
      reason: '',
    },
    {
      id: 'handoff-3',
      area: 'bar',
      title: 'Инвентарь сиропов',
      criticality: 'medium',
      checked: true,
      reason: 'Передано ночной смене',
    },
  ],
  requests: [
    {
      id: 'req-1',
      category: 'kitchen',
      item: 'Авокадо',
      remaining: '2 кг',
      needed: '8 кг',
      comment: 'Нужна поставка к утру',
      createdAt: now.toISOString(),
    },
  ],
  employees: [],
  loginEmployees: [],
  session: {
    bootstrapped: null,
    token: null,
    me: null,
  },
  shiftReflections: [],
  specialStarAwards: [],
  bonusAwards: [],
  revenueGoals: {
    weeklyRevenueTarget: 580000,
    monthlyRevenueTarget: 2450000,
    monthlyAverageCheckStart: 3200,
    monthlyAverageCheckTarget: 3600,
  },
  goals: {
    activePeriod: createGoalPeriod('month', now),
    metric: {
      type: 'custom',
      unit: 'pts',
      targetValue: 120,
      currentValue: 18,
      label: 'Командный прогресс',
    },
    viewerEmployeeId: null,
    contributions: normalizeGoalContributions({
      ...createEmptyGoalContributions(now.toISOString()),
      waiters: {
        department: 'waiters',
        pointsEarned: 8,
        percent: 0,
        lastUpdatedAt: now.toISOString(),
      },
      bar: {
        department: 'bar',
        pointsEarned: 5,
        percent: 0,
        lastUpdatedAt: now.toISOString(),
      },
      kitchen: {
        department: 'kitchen',
        pointsEarned: 4,
        percent: 0,
        lastUpdatedAt: now.toISOString(),
      },
      hookah: {
        department: 'hookah',
        pointsEarned: 1,
        percent: 0,
        lastUpdatedAt: now.toISOString(),
      },
      other: {
        department: 'other',
        pointsEarned: 0,
        percent: 0,
        lastUpdatedAt: null,
      },
    }),
    tasks: [
      {
        id: 'goal-global-reviews',
        scope: 'global',
        title: 'Отзывы гостей',
        description: 'Собираем живые отзывы, чтобы поднять общий прогресс недели.',
        department: 'waiters',
        points: 1,
        targetCount: 12,
        progressCount: 4,
        status: 'in_progress',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-global-cocktail',
        scope: 'global',
        title: 'Коктейль недели',
        description: 'Каждая продажа продвигает общую цель периода.',
        department: 'bar',
        points: 2,
        targetCount: 8,
        progressCount: 2,
        status: 'in_progress',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-global-dessert',
        scope: 'global',
        title: 'Десерт дня',
        description: 'Подсвечиваем десерт в каждой второй рекомендации.',
        department: 'waiters',
        points: 1,
        targetCount: 10,
        progressCount: 2,
        status: 'in_progress',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-global-new-dish',
        scope: 'global',
        title: 'Тест новой подачи',
        description: 'Кухня проводит прогоны и подтверждает стабильный выход.',
        department: 'kitchen',
        points: 2,
        targetCount: 5,
        progressCount: 1,
        status: 'in_progress',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-role-waiter-upsell',
        scope: 'role',
        role: 'waiter',
        title: 'Апселл на десерт',
        description: 'Каждые 5 продаж десертов добавляют заметный вклад в общую цель.',
        department: 'waiters',
        points: 1,
        targetCount: 5,
        progressCount: 1,
        status: 'in_progress',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-role-bartender-signature',
        scope: 'role',
        role: 'bartender',
        title: 'Фирменный коктейль',
        description: 'Продвигаем авторский напиток смены.',
        department: 'bar',
        points: 2,
        targetCount: 4,
        progressCount: 1,
        status: 'in_progress',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-role-chef-standard',
        scope: 'role',
        role: 'chef',
        title: 'Стабильный выход блюда',
        description: 'Каждый подтвержденный прогон новой позиции приносит баллы.',
        department: 'kitchen',
        points: 2,
        targetCount: 3,
        progressCount: 1,
        status: 'in_progress',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-role-hookah-special',
        scope: 'role',
        role: 'hookah',
        title: 'Авторский кальян',
        description: 'Спец-задача для кальянного отдела.',
        department: 'hookah',
        points: 3,
        targetCount: 3,
        progressCount: 0,
        status: 'todo',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-personal-masha',
        scope: 'personal',
        employeeId: 'emp-masha',
        title: 'Личное внимание к гостям',
        description: 'Собери 3 персональные благодарности от столов за смену.',
        department: 'waiters',
        points: 2,
        targetCount: 3,
        progressCount: 1,
        status: 'in_progress',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-personal-ilya',
        scope: 'personal',
        employeeId: 'emp-ilya',
        title: 'Чистый бар без хвостов',
        description: 'Закрой смену без возвратов по барному чек-листу.',
        department: 'bar',
        points: 3,
        targetCount: 1,
        progressCount: 0,
        status: 'todo',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
      {
        id: 'goal-personal-anton',
        scope: 'personal',
        employeeId: 'emp-anton',
        title: 'Темп кухни',
        description: 'Проведи смену без возвратов по качеству на горячем цехе.',
        department: 'kitchen',
        points: 3,
        targetCount: 1,
        progressCount: 0,
        status: 'todo',
        createdAt: now.toISOString(),
        updatedAt: now.toISOString(),
      },
    ],
  },
  dailyBusinessMetrics: [
    {
      dateKey: localDateKeyAt(0),
      revenueActual: 78500,
      averageCheckTarget: 2350,
      averageCheckActual: 2210,
      updatedAt: now.toISOString(),
    },
    {
      dateKey: localDateKeyAt(-1),
      revenueActual: 72400,
      averageCheckTarget: 2250,
      averageCheckActual: 2310,
      updatedAt: isoAt(-1, 23, 35),
    },
    {
      dateKey: localDateKeyAt(-2),
      revenueActual: 69300,
      averageCheckTarget: 2200,
      averageCheckActual: 2180,
      updatedAt: isoAt(-2, 23, 10),
    },
  ],
  timeEntries: [
    {
      id: 'time-owner-1',
      userId: 'emp-owner-main',
      role: 'owner',
      startAt: isoAt(-3, 11, 20),
      endAt: isoAt(-3, 21, 10),
      earlyStart: false,
      earlyReason: null,
      createdAt: isoAt(-3, 11, 20),
    },
    {
      id: 'time-1',
      userId: 'emp-anton',
      role: 'chef',
      startAt: isoAt(0, 11, 25),
      endAt: isoAt(0, 19, 10),
      earlyStart: false,
      earlyReason: null,
      createdAt: isoAt(0, 11, 25),
    },
    {
      id: 'time-2',
      userId: 'emp-anton',
      role: 'chef',
      startAt: isoAt(-2, 10, 45),
      endAt: isoAt(-2, 20, 5),
      earlyStart: true,
      earlyReason: 'Ранний старт из-за поставки мяса',
      createdAt: isoAt(-2, 10, 45),
    },
    {
      id: 'time-3',
      userId: 'emp-masha',
      role: 'waiter',
      startAt: isoAt(0, 11, 20),
      endAt: isoAt(0, 22, 50),
      earlyStart: false,
      earlyReason: null,
      createdAt: isoAt(0, 11, 20),
    },
    {
      id: 'time-4',
      userId: 'emp-masha',
      role: 'waiter',
      startAt: isoAt(-4, 11, 20),
      endAt: isoAt(-4, 23, 20),
      earlyStart: false,
      earlyReason: null,
      createdAt: isoAt(-4, 11, 20),
    },
    {
      id: 'time-5',
      userId: 'emp-ilya',
      role: 'bartender',
      startAt: isoAt(-1, 11, 10),
      endAt: isoAt(-1, 23, 25),
      earlyStart: true,
      earlyReason: 'Подготовка бара к мероприятию',
      createdAt: isoAt(-1, 11, 10),
    },
    {
      id: 'time-6',
      userId: 'emp-ilya',
      role: 'bartender',
      startAt: isoAt(-6, 11, 30),
      endAt: isoAt(-6, 22, 45),
      earlyStart: false,
      earlyReason: null,
      createdAt: isoAt(-6, 11, 30),
    },
  ],
};
